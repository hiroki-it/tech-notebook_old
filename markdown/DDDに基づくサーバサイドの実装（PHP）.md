...
- [02-01. MVCとは](#02-01-mvcとは)
        - [◆ MVCからドメイン駆動設計への発展](#◆-mvcからドメイン駆動設計への発展)
- [03-01. ドメイン駆動設計とは](#03-01-ドメイン駆動設計とは)
        - [◆ドメインエキスパートとユビキタス言語](#◆ドメインエキスパートとユビキタス言語)
        - [◆ 戦略的設計](#◆-戦略的設計)
        - [◆ 戦術的設計](#◆-戦術的設計)
        - [◆ ドメイン駆動設計の派生型](#◆-ドメイン駆動設計の派生型)
...
- [03-03. Dependency Inversion Principle（依存性逆転の原則）](#03-03-dependency-inversion-principle依存性逆転の原則)
        - [◆ DIPとは](#◆-dipとは)
        - [◆ DIPに基づくドメイン駆動設計](#◆-dipに基づくドメイン駆動設計)
- [04-01. Application層](#04-01-application層)
        - [◆ Controllerの責務](#◆-controllerの責務)
        - [◆ Caster（データ構造変換クラス）の責務](#◆-casterデータ構造変換クラスの責務)
- [05-01. Domain層のRepository](#05-01-domain層のrepository)
- [05-02. Entity](#05-02-entity)
        - [◆ 責務](#◆-責務)
        - [◆ RouteEntityとは](#◆-routeentityとは)
- [05-03. ValueObject](#05-03-valueobject)
        - [◆ 責務](#◆-責務-1)
        - [◆（1）一意に識別できるプロパティをもたず、対象のユビキタス言語に関するプロパティをメソッドを持つ](#◆1一意に識別できるプロパティをもたず対象のユビキタス言語に関するプロパティをメソッドを持つ)
        - [◆（2）プロパティの不変性](#◆2プロパティの不変性)
        - [◆（3）概念的な統一体](#◆3概念的な統一体)
        - [◆（4）オブジェクトの交換可能性](#◆4オブジェクトの交換可能性)
        - [◆（5）オブジェクト間の等価性](#◆5オブジェクト間の等価性)
        - [◆（6）メソッドによってオブジェクトの状態が変わらない](#◆6メソッドによってオブジェクトの状態が変わらない)
        - [◆ EntityとValueObjectのどちらとして、モデリング／実装すべきなのか？](#◆-entityとvalueobjectのどちらとしてモデリング／実装すべきなのか)
- [05-04. TypeCode（標準型）](#05-04-typecode標準型)
        - [◆ 責務](#◆-責務-2)
        - [◆ EnumによるTypeCodeの実装](#◆-enumによるtypecodeの実装)
- [05-05. Id](#05-05-id)
- [05-06. Service](#05-06-service)
- [06-01. Infrastructure層のRepository](#06-01-infrastructure層のrepository)
        - [◆ 集約の構成／データベースへのデータ追加](#◆-集約の構成／データベースへのデータ追加)
        - [◆ データベースからのデータ取得／集約の再構成](#◆-データベースからのデータ取得／集約の再構成)
