...
- [05-02. AWS仮想サーバへのデプロイ](#05-02-aws仮想サーバへのデプロイ)
...
- [05-03. AWSにおけるグローバルネットワーク構成](#05-03-awsにおけるグローバルネットワーク構成)
...
- [05-04. AWSにおけるプライベートネットワーク構成](#05-04-awsにおけるプライベートネットワーク構成)
    - [:pushpin: VPC：Virtual Private Cloud ⇒ プライベートネットワークに相当](#pushpin-vpcvirtual-private-cloud-⇒-プライベートネットワークに相当)
    - [:pushpin: Route Table ⇒ ルータに相当](#pushpin-route-table-⇒-ルータに相当)
    - [:pushpin: ELB：Elastic Load Balancing](#pushpin-elbelastic-load-balancing)
    - [:pushpin: セキュリティグループ ⇒ ファイアウォールに相当](#pushpin-セキュリティグループ-⇒-ファイアウォールに相当)
...
![IPアドレスのホスト部とネットワーク部](/Users/hiroki/Documents/Drive 1st/Programming/tech-notebook/image/IPアドレスのホスト部とネットワーク部.png)
...
『1』あるいは『0』でIPアドレスのネットワーク部とホスト部を定義し、IPアドレスの後ろに記述する方法（**【具体例】**192.168.42.23/24）。ネットワーク部を何bitとするかを『1』、ホスト部を『0』で表現する。二進数形式、IPアドレス形式、CIDR形式による表現方法がある。

|             二進数形式              | IPアドレス形式  | CIDR形式 |
| :---------------------------------: | :-------------: | :------: |
...
クラウドメッセージキューとして働く。異なるVPC間でも、メッセージキューを同期できる。仮想サーバで生成されたメッセージは、一旦SQSに追加される。コマンドによってバッチが実行され、メッセージが取り出される。その後、例えば、バッチ処理によってメッセージからデータが取り出されてファイルが生成され、S3に保存されるような処理が続く。
...
クラウドプライベートネットワークとして働く。プライベートIPアドレスが割り当てられた、VPCと呼ばれるプライベートネットワークを仮想的に構築することができる。異なるAvailability Zoneに渡ってkクラウドWebサーバを立ち上げることによって、仮想サーバをデュアル化することできる。

![VPCが提供できるネットワークの範囲](https://raw.githubusercontent.com/Hiroki-IT/tech-notebook/master/markdown/image/VPCが提供できるネットワークの範囲.png)

### :pushpin: Route Table ⇒ ルータに相当

クラウドルータとして働く。ルータについては、NATとNAPTを参照せよ。
...
### :pushpin: セキュリティグループ ⇒ ファイアウォールに相当
...
- **パブリックサブネット**
  ...
  非武装地帯に相当し、クラウドWebサーバが構築される。

- **プライベートサブネット**

  内部ネットワークに相当し、Amazon Aurora（クラウドデータベース）などが構築される。
