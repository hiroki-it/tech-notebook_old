...
Webサーバにおける以下のディレクトリで、これらのログを確認できる
...
class StaffAPI
{
  ...
  {
    // Staffに関するGET送信を、StaffControllerのデータ取得メソッドと紐づける。
  }
  
  
  return function post($request)
  {
   	// Staffに関するPOST送信を、StaffControllerのデータ新規作成メソッドと紐づける。 
  }
}
```
...
# 03-03. 仮想サーバの構築、ソフトウェアインストール、サーバ操作
...
# 03-04. コンテナの構築、ソフトウェアインストール、操作

![Dockerアーキテクチャ](https://raw.githubusercontent.com/Hiroki-IT/tech-notebook/master/markdown/image/Dockerアーキテクチャ.png)

### ◆ クライアントとDockerデーモンの間のリモート接続

クライアントは、ターミナルで```docker```コマンドを実行し、dockerデーモンを経由して、コンテナの構築、ソフトウェアインストール、操作を行う。
