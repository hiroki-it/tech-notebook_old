...
- [01-01. ネットワークの全体像](#01-01-ネットワークの全体像)
    - [◆ インターネット、WAN、LAN](#◆-インターネットwanlan)
    - [◆ WAN、LANの具体例](#◆-wanlanの具体例)
    - [◆ WANの種類と歴史](#◆-wanの種類と歴史)
    - [◆ グローバルネットワークとプライベートネットワーク](#◆-グローバルネットワークとプライベートネットワーク)
    - [◆ プライベートネットワークにおけるセグメント](#◆-プライベートネットワークにおけるセグメント)
    - [◆ ネットワークにおけるデータ通信方法の種類](#◆-ネットワークにおけるデータ通信方法の種類)
- [02-01. OSI参照モデルとTCP階層モデル](#02-01-osi参照モデルとtcp階層モデル)
    - [◆ データの作成、ヘッダ情報追加、カプセル化](#◆-データの作成ヘッダ情報追加カプセル化)
    - [◆ OSI参照モデルにおいて各概念層で追加されるヘッダ情報の内容](#◆-osi参照モデルにおいて各概念層で追加されるヘッダ情報の内容)
    - [◆ プロトコルの分類と扱われる階層](#◆-プロトコルの分類と扱われる階層)
- [02-02. 通信機器におけるヘッダ情報認識](#02-02-通信機器におけるヘッダ情報認識)
    - [◆ 各概念層の実際の通信機器の対応関係](#◆-各概念層の実際の通信機器の対応関係)
    - [◆ 通信機器における各層のヘッダ情報の認識](#◆-通信機器における各層のヘッダ情報の認識)
- [02-03. アプリケーション層におけるデータ（メッセージ）の作成](#02-03-アプリケーション層におけるデータメッセージの作成)
    - [◆ RESTful APIに対するリクエストメッセージの構造](#◆-restful-apiに対するリクエストメッセージの構造)
    - [◆ （補足）SOAP APIに対するリクエストメッセージの構造](#◆-補足soap-apiに対するリクエストメッセージの構造)
    - [◆ レスポンスメッセージの構造](#◆-レスポンスメッセージの構造)
    - [◆ HTTPステータスの種類](#◆-httpステータスの種類)
- [02-04. アプリケーション層におけるメールデータの作成](#02-04-アプリケーション層におけるメールデータの作成)
    - [◆ メール送信用プロトコル](#◆-メール送信用プロトコル)
    - [◆ メール受信用プロトコル](#◆-メール受信用プロトコル)
- [02-05. トランスポート層におけるヘッダ情報追加](#02-05-トランスポート層におけるヘッダ情報追加)
    - [◆ TCPによるヘッダ情報の追加と識別の仕組み](#◆-tcpによるヘッダ情報の追加と識別の仕組み)
    - [◆ ポートスキャナ](#◆-ポートスキャナ)
    - [◆ ポート番号の種類](#◆-ポート番号の種類)
- [02-06. インターネット層におけるヘッダ情報追加](#02-06-インターネット層におけるヘッダ情報追加)
    - [◆ IPパケットのヘッダ情報を用いた宛先認識](#◆-ipパケットのヘッダ情報を用いた宛先認識)
    - [◆ IPv4アドレスの種類](#◆-ipv4アドレスの種類)
    - [◆ IPアドレスとbitの関係](#◆-ipアドレスとbitの関係)
    - [◆ IPv4アドレスのクラス](#◆-ipv4アドレスのクラス)
    - [◆ URLとメールアドレスの構造](#◆-urlとメールアドレスの構造)
    - [◆ 完全修飾ドメイン名とグローバルIPアドレスのマッピング](#◆-完全修飾ドメイン名とグローバルipアドレスのマッピング)
- [02-07. ルータの種類](#02-07-ルータの種類)
    - [◆ NAT：Network Address TranslationによるIPアドレスv4の変換](#◆-natnetwork-address-translationによるipアドレスv4の変換)
    - [◆ NAPT：Network Address Port Translationによるポート番号の変換](#◆-naptnetwork-address-port-translationによるポート番号の変換)
- [03-01. Webシステムを構成する主要な3層構造](#03-01-webシステムを構成する主要な3層構造)
    - [◆ WebサーバソフトウェアをもつWebサーバ](#◆-webサーバソフトウェアをもつwebサーバ)
    - [◆ APサーバソフトウェアをもつAPサーバ](#◆-apサーバソフトウェアをもつapサーバ)
    - [◆ データベース管理システムをもつDBサーバ](#◆-データベース管理システムをもつdbサーバ)
    - [◆ サーバの処理能力向上](#◆-サーバの処理能力向上)
- [03-02. Webシステムの構成方法](#03-02-webシステムの構成方法)
    - [◆ Dualシステム](#◆-dualシステム)
    - [◆ Duplexシステム](#◆-duplexシステム)
    - [◆ システムの稼働率](#◆-システムの稼働率)
- [03-03. Web API](#03-03-web-api)
    - [◆ RESTful API](#◆-restful-api)
    - [◆ SOAP API](#◆-soap-api)
- [03-04. サーバ間のリクエストとレスポンス](#03-04-サーバ間のリクエストとレスポンス)
    - [◆ サーバ間のデータ通信に要する時間](#◆-サーバ間のデータ通信に要する時間)
    - [◆ Proxyサーバ](#◆-proxyサーバ)
    - [◆ Proxyサーバ（Fw/Re）、DNSサーバ、Webサーバによる名前解決](#◆-proxyサーバfwrednsサーバwebサーバによる名前解決)
- [03-05. キャッシュ、Cookie、セッションID](#03-05-キャッシュcookieセッションid)
    - [◆ キャッシュ](#◆-キャッシュ)
    - [◆ CookieプロトコルによるCookie情報の送受信](#◆-cookieプロトコルによるcookie情報の送受信)
    - [◆ CookieプロトコルによるセッションIDの送受信](#◆-cookieプロトコルによるセッションidの送受信)
- [04-01. サーバ仮想化とコンテナ仮想化](#04-01-サーバ仮想化とコンテナ仮想化)
    - [◆ ホスト型仮想化](#◆-ホスト型仮想化)
    - [◆ ハイパーバイザー型仮想化](#◆-ハイパーバイザー型仮想化)
    - [◆ コンテナ型仮想化](#◆-コンテナ型仮想化)
- [04-02. 各仮想化のパフォーマンスの比較](#04-02-各仮想化のパフォーマンスの比較)
    - [◆ 起動速度の違い](#◆-起動速度の違い)
    - [◆ 処理速度の違い](#◆-処理速度の違い)
- [04-03. 仮想サーバの構築、ソフトウェアインストール、サーバ操作](#04-03-仮想サーバの構築ソフトウェアインストールサーバ操作)
    - [◆ Vagrant](#◆-vagrant)
    - [◆ Provider](#◆-provider)
    - [◆ Provisioner](#◆-provisioner)
- [04-04. コンテナの構築、ソフトウェアインストール、コンテナ操作](#04-04-コンテナの構築ソフトウェアインストールコンテナ操作)
    - [◆ Dockerクライアント](#◆-dockerクライアント)
    - [◆ Dockerホスト](#◆-dockerホスト)
    - [◆ レジストリ](#◆-レジストリ)
- [05-01. オンプレミスとクラウドコンピューティング](#05-01-オンプレミスとクラウドコンピューティング)
    - [◆ On premise](#◆-on-premise)
    - [◆ Iaas：Infrastructure as a Service](#◆-iaasinfrastructure-as-a-service)
    - [◆ Paas：Platform as a Service](#◆-paasplatform-as-a-service)
    - [◆ Saas：Software as a Service](#◆-saassoftware-as-a-service)
- [05-02. masterブランチ変更後の自動テスト](#05-02-masterブランチ変更後の自動テスト)
    - [◆ Continuous Integratorにおける自動テスト](#◆-continuous-integratorにおける自動テスト)
- [05-03. 自動テスト後のAWS仮想サーバへのデプロイ](#05-03-自動テスト後のaws仮想サーバへのデプロイ)
    - [◆ インスタンスへのデプロイにデプロイツールを用いる場合](#◆-インスタンスへのデプロイにデプロイツールを用いる場合)
    - [◆ インスタンスへのデプロイにAWS CodeDeployを用いる場合](#◆-インスタンスへのデプロイにaws-codedeployを用いる場合)
- [05-04. AWSによるWebサービスのリリース](#05-04-awsによるwebサービスのリリース)
    - [◆ RegionとAvailability Zone](#◆-regionとavailability-zone)
    - [◆ Route 53](#◆-route-53)
    ...
    - [◆ Dualシステム（再掲）](#◆-dualシステム再掲)
- [05-05. GCPによるWebサービスのリリース](#05-05-gcpによるwebサービスのリリース)
    - [◆ GAE：Google App Engine：GAE](#◆-gaegoogle-app-enginegae)
    - [◆ GCE：Google Compute Engine](#◆-gcegoogle-compute-engine)
...
『```ルート + ? + クエリストリング```』の形でHTTPリクエストを送る方法。URLに情報が記述されるため、履歴で確認できてしまう。リクエストメッセージは、以下の要素に分類できる。

```
http://127.0.0.1/testform.php + ? + クエリストリング
```

以下では、Web APIのうち、特にRESTfulAPIに対して送信するためのリクエストメッセージの構造を説明する。
...
- **POST送信の場合**
  ...
  メッセージボディに記述してリクエストメッセージを送る方法。以下では、Web APIのうち、特にRESTfulAPIに対して送信するためのリクエストメッセージの構造を説明する。メッセージボディに情報が記述されるため、履歴では確認できない。リクエストメッセージは、以下の要素に分類できる。
...
- **グローバルIPアドレス**
  ...
  プロバイダが提供するIPv4アドレス。パブリックネットワーク内に同じIPv4アドレスは存在せず、Network Information Centerへの使用申請が必要。プライベートIPアドレスの番号でなければ、グローバルIPアドレスである。NATはグローバルIPアドレスを持っており、プライベートIPアドレスと相互変換する。
...
例えば、プライベートIPアドレスの４つの各区分が１Byteの容量をもち、IPアドレス全体で４Byteの容量をもつ。ちなみに、172から始まるIPアドレスは、クラスBである。

![IPアドレスとbitの関係](image/IPアドレスとbitの関係.png)



### ◆ IPv4アドレスのクラス

上位8ビットの数字によって、クラス分けされている。
...
- **リクエスト時のルータにおける変換**
  ...
  NATはグローバルIPアドレスを持っており、パケットのヘッダ情報におけるプライベートIPアドレスとグローバルIPアドレスを相互変換する。例えば、GoogleでWebページを見ながら、Gmailアプリを起動している場合、リクエストにおけるパケット情報として…
...
NATはグローバルIPアドレスを持っており、NATと同様にプライベートIPアドレスとグローバルIPアドレスの間を相互変換するだけでなく、ポート番号も変換する。
...
### ◆ WebサーバソフトウェアをもつWebサーバ

Webサーバソフトウェア（Apache、Nginxなど）がインストールされている。Webブラウザから静的コンテンツのリクエストがあった場合に、アプリケーションサーバにリクエストを行う。アプリケーションサーバからレスポンスを受け取り、Webブラウザにレスポンスを行う。
...
APサーバソフトウェア（PHP-FPM、NGINX Unit）がインストールされている。Webサーバから動的コンテンツのリクエストがあった場合に、プログラミング言語を言語プロセッサで翻訳し、DBサーバにリクエストを行う。DBサーバからのレスポンスを受け取り、Webサーバに動的なコンテンツのレスポンスを行う。
...
### ◆ Proxyサーバ（Fw/Re）、DNSサーバ、Webサーバによる名前解決
...
1. 最初、サーバは、レスポンスヘッダ情報のCookieプロトコルにCookie情報を埋め込んで送信する。
2. ブラウザは、そのCookie情報を保存する。
3. 2回目以降のリクエストでは、ブラウザは、リクエストヘッダ情報のCookieプロトコルにCookie情報を埋め込んでサーバに送信する。サーバは、Cookie情報に紐づくクライアントのデータをReadする。
...
1. 最初、サーバは、レスポンスヘッダ情報のCookieプロトコルにセッションIDを埋め込んで送信する。
2. ブラウザは、そのセッションIDを保存する。
3. 2回目以降のリクエストでは、ブラウザは、リクエストヘッダ情報のCookieプロトコルにセッションIDを埋め込んでサーバに送信する。サーバは、セッションIDに紐づくクライアントのデータをReadする。

![セッションIDの仕組み](https://raw.githubusercontent.com/Hiroki-IT/tech-notebook/master/markdown/image/セッションIDの仕組み.png)



## 04-01. サーバ仮想化とコンテナ仮想化

自身の開発環境でWebサイトを動かしたい場合、まず、パソコン内にLinux環境のWebサーバ、APサーバ、DBサーバなどの物理サーバを仮想的に構築する。そして、自身のパソコンをクライアント、各仮想サーバをリクエスト先に見立てて、SSHプロトコルを用いてこれらのサーバにリモートログインし、操作する。仮想環境の構築方法にはいくつか種類がある。
...
## 04-03. 仮想サーバの構築、ソフトウェアインストール、サーバ操作
...
ProviderとProvisionerによる仮想サーバの構築、ソフトウェアインストール、サーバ操作、を自動化できるソフトウェア。チームメンバーが別々に仮想サーバを構築する場合、ProviderとProvisionerの処理によって作られる仮想サーバの環境に、違いが生じてしまう。Vagrantを使う場合、ProviderとProvisionerによる処理方法は、Vagrantfileに記述されている。このために、Vagrantを用いれば、チームメンバーが同じソフトウェアの下で、仮想サーバを構築し、ソフトウェアをインストールし、サーバを操作することができる。サーバの情報は、ENVファイルで以下の様に管理する。
...
Vagrantによって操作されるソフトウェア。基本ソフトウェアにおける制御プログラムや一連のハードウェアを仮想的に構築する。構築方法の違いによって、『ホスト型』、『ハイパーバイザ型』に分類できる。
...
## 04-04. コンテナの構築、ソフトウェアインストール、コンテナ操作
...
コンテナにインストールしなければならないソフトウェアのテンプレートのこと。チーム開発では、共有されたイメージを元に、同じ環境設定が行われたコンテナを構築することができる。以下に、例を示す。

| インストールするべきソフトウェア | ソフトウェア例 |
| :------------------------------: | -------------- |
|              **OS**              | CentOS         |
|        **NoSQL（非RDB）**        | Redis          |
|            **RDBMS**             | MySQL          |
...
## 05-01. オンプレミスとクラウドコンピューティング

ベンダーが、システムを稼働させるために必要なソフトウェアとハードウェアをどこまで提供するかによって、サービスの名称が異なる。

![Iaas、Paas、Saasの比較](https://raw.githubusercontent.com/Hiroki-IT/tech-notebook/master/markdown/image/Iaas、Paas、Saasの比較.JPG)
...
## 05-03. 自動テスト後のAWS仮想サーバへのデプロイ

### ◆ インスタンスへのデプロイにデプロイツールを用いる場合

デプロイするためには、デプロイサーバーが必要である。デプロイツールはデプロイサーバを仮想的に構築することもできる。本番環境（ここではクラウドWebサーバ）にGitをインストールして```pull```を行い、GitHubから仮想デプロイサーバにmasterブランチの状態を取り込む。
...
![Capistranoを用いた自動デプロイ](https://raw.githubusercontent.com/Hiroki-IT/tech-notebook/master/markdown/image/Capistranoを用いた自動デプロイ.png)
...
2016年1月6日時点では、以下のRegionに物理サーバのデータセンターがある。さらに、各データセンターは物理的に独立したAvailability Zoneというロケーションから構成されている。東京Regionには、3つのAvailability Zoneがある。
...
クラウドプライベートネットワークにおけるセグメントとして働く。パブリックサブネットは非武装地帯に相当し、インスタンス（クラウドWebサーバ）が構築される。プライベートサブネットは内部ネットワークに相当し、Amazon Aurora（クラウドデータベース）などが構築される。
