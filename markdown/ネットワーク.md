...
- [02-03. アプリケーション層におけるWebデータの作成](#02-03-アプリケーション層におけるwebデータの作成)
        - [◆ プロトコルの分類と扱われる階層（再掲）](#◆-プロトコルの分類と扱われる階層再掲)
        - [◆ 通信機器における各層のヘッダ情報の認識（再掲）](#◆-通信機器における各層のヘッダ情報の認識再掲)
        - [◆ Web APIのアーキテクチャスタイルと設計](#◆-web-apiのアーキテクチャスタイルと設計)
        - [◆ SOAPにおける送信方法](#◆-soapにおける送信方法)
        - [◆ RESTにおける送信方法](#◆-restにおける送信方法)
        - [◆ HTTPステータスの種類](#◆-httpステータスの種類)
- [02-04. アプリケーション層におけるメールデータの作成](#02-04-アプリケーション層におけるメールデータの作成)
        - [◆ プロトコルの分類と扱われる階層（再掲）](#◆-プロトコルの分類と扱われる階層再掲-1)
        - [◆ 通信機器における各層のヘッダ情報の認識（再掲）](#◆-通信機器における各層のヘッダ情報の認識再掲-1)
        - [◆ メール送信用プロトコル](#◆-メール送信用プロトコル)
        - [◆ メール受信用プロトコル](#◆-メール受信用プロトコル)
- [02-05. トランスポート層におけるヘッダ情報追加](#02-05-トランスポート層におけるヘッダ情報追加)
        - [◆ プロトコルの分類と扱われる階層（再掲）](#◆-プロトコルの分類と扱われる階層再掲-2)
        - [◆ 通信機器における各層のヘッダ情報の認識（再掲）](#◆-通信機器における各層のヘッダ情報の認識再掲-2)
        - [◆ TCPによるヘッダ情報の追加と識別の仕組み](#◆-tcpによるヘッダ情報の追加と識別の仕組み)
        - [◆ ポートスキャナ](#◆-ポートスキャナ)
        - [◆ ポート番号の種類](#◆-ポート番号の種類)
- [02-06. インターネット層におけるヘッダ情報追加](#02-06-インターネット層におけるヘッダ情報追加)
        - [◆ プロトコルの分類と扱われる階層（再掲）](#◆-プロトコルの分類と扱われる階層再掲-3)
        - [◆ 通信機器における各層のヘッダ情報の認識（再掲）](#◆-通信機器における各層のヘッダ情報の認識再掲-3)
...
- [03-03. Webシステムの構成方法](#03-03-webシステムの構成方法)
        - [◆ Dualシステム](#◆-dualシステム)
...
### ◆ Web APIのアーキテクチャスタイルと設計

『SOAP』と『REST』がある。RESTに基づく設計をRESTfulという。
...
- **GET送信によるリクエスト情報**
  ...
  『```ルート + ? + クエリストリング```』の形でHTTPリクエストを送る方法。URLに情報が記述されるため、履歴で確認できてしまう。リクエスト情報は、以下の要素に分類できる。
...
1. リクエスト行
2. リクエストヘッダー：リクエストの詳細情報（SSLによって暗号化される部分）
3. 送信方法：GET
4. リクエストの対象：ファイルへのパス
5. HTTPプロトコルのバージョン
...
1. HTTPステータス
2. レスポンスヘッダー：レスポンスの詳細情報がある（SSLによって暗号化される部分）
3. 空白行：ヘッダーとボディを分ける
...
ユーザのメールアドレスやパスワードは、ブラウザにCookieとして保存される。



### ◆ セッションID

ユーザのメールアドレスやパスワードは、WebサーバにセッションIDとして保存される。
