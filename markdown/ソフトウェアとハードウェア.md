...
# 01-03. 基本ソフトウェアの制御プログラム（別名：カーネル）
...
![到着順方式_1](C:\Projects\summary_notes\SummaryNotes\Image\到着順方式_1.png)

![到着順方式_2](C:\Projects\summary_notes\SummaryNotes\Image\到着順方式_2.png)

- **Round robin 方式**

  Round robinは、『総当たり』の意味。一定時間（タイムクウォンタム）ごとに、実行状態にあるタスクが強制的に実行可能状態の待ち行列に登録される。交代するように、他のタスクがディスパッチされる。
  
  ![ラウンドロビン方式](C:\Projects\summary_notes\SummaryNotes\Image\ラウンドロビン方式.png)

**【具体例】**

生成されたタスクの到着時刻と処理時間は以下のとおりである。強制的なディスパッチは、『20秒』ごとに起こるとする。

![優先順方式_1](C:\Projects\summary_notes\SummaryNotes\Image\優先順方式_1.png)

20秒時点で、Aは、実行状態から実行可能状態の待ち行列に登録される。続けて、Bは、逆に実行可能状態から実行状態にディスパッチされる。

![優先順方式_2](C:\Projects\summary_notes\SummaryNotes\Image\優先順方式_2.png)
...
![Javaによる言語処理_1](C:\Projects\summary_notes\SummaryNotes\Image\Javaによる言語処理_1.png)

![矢印_80x82](C:\Projects\summary_notes\SummaryNotes\Image\矢印_80x82.jpg)

オブジェクトコードを、JVM：Java Virtual Machine内の仕組みによって、機械語に翻訳する。

![Javaによる言語処理_2](C:\Projects\summary_notes\SummaryNotes\Image\Javaによる言語処理_2.png)

![矢印_80x82](C:\Projects\summary_notes\SummaryNotes\Image\矢印_80x82.jpg)

結果的に、OSに依存せずに、命令を実行できる。

![Javaによる言語処理_3](C:\Projects\summary_notes\SummaryNotes\Image\Javaによる言語処理_3.png)
