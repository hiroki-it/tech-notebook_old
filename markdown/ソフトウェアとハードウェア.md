...
- [03-04. コンテナの構築](#03-04-コンテナの構築)
    ...
    - [:pushpin: Docker Composeによるサービスのビルド，コンテナの構築，コンテナの起動](#pushpin-docker-composeによるサービスのビルドコンテナの構築コンテナの起動)
...
| コマンド                             | 処理                                                         | 備考                                                         |
| :----------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **```build --no-cache```**           | キャッシュ無しでサービスビルド                               |                                                              |
| **```up -d```**                      | ・サービスビルド<br>・全コンテナ構築<br>・コンテナ起動       | すでにコンテナがある場合<br>それを再起動する                 |
| **```run```**                        | ・サービスビルド<br>・指定／依存コンテナ構築<br>・コンテナ起動 | すでにコンテナがあっても<br>それを残して構築／起動．<br>（注）不要なコンテナが増える． |
| **```start```**                      | 既存コンテナ起動                                             |                                                              |
| **```stop```**                       | 起動中コンテナ停止                                           |                                                              |
| **```exec -it {コンテナ名} bash```** | 起動中コンテナにリモート接続                                 |                                                              |
| **```ps```**                         | 起動中コンテナの一覧で表示                                   |                                                              |
...
| コマンド                      | 処理                             |
| ----------------------------- | -------------------------------- |
| **```search {イメージ名}```** | イメージを検索                   |
| **```pull {イメージ名}```**   | イメージをインストール           |
| **```image ls```**            | インストールされたイメージを確認 |
...
各Dockerイメージのインストールと設定に必要な一連のコマンドが記述された実行ファイル．Dockerfileを用いることで，以下のようなメリットがある．インストールはDockerHubからインストールする．以下のコマンドで，指定したDockerfileのみをビルドできる．
...
- **Dockerfileの代わりにAnsibleを使う場合**

- **Docker Hub**



### :pushpin: Docker Composeによるサービスのビルド，コンテナの構築，コンテナの起動

インストールされたDockerイメージから，サービスのビルド，コンテナの構築，コンテナの起動，を行うための管理ツール．
