...
- [03-03. 仮想マシン内のサーバの構築](#03-03-仮想マシン内のサーバの構築)
    - [:pushpin: VagrantによるProviderとProvisionerの操作](#pushpin-vagrantによるproviderとprovisionerの操作)
    - [:pushpin: Providerによる仮想マシン内のサーバの構築](#pushpin-providerによる仮想マシン内のサーバの構築)
...
- [03-04. コンテナの構築](#03-04-コンテナの構築)
    ...
    - [:pushpin: コマンドでコンテナを操作するまでの手順まとめ](#pushpin-コマンドでコンテナを操作するまでの手順まとめ)
    - [:pushpin: ベースイメージのインストール](#pushpin-ベースイメージのインストール)
    - [:pushpin: Dockerイメージのビルド](#pushpin-dockerイメージのビルド)
    - [:pushpin: コンテナレイヤーの生成とコンテナの構築](#pushpin-コンテナレイヤーの生成とコンテナの構築)
    - [:pushpin: Dockerクライアントによるコンテナの操作](#pushpin-dockerクライアントによるコンテナの操作)
    - [:pushpin: docker-compose.ymlによる手順の全自動化](#pushpin-docker-composeymlによる手順の全自動化)
...
ProviderとProvisionerを操作し，仮想マシン内のサーバ，ソフトウェアのインストール，を自動化するソフトウェア．チームメンバーが別々に仮想マシン内に仮想サーバを構築する場合，ProviderとProvisionerの処理によって作られる仮想サーバの環境に，違いが生じてしまう．Vagrantを使う場合，ProviderとProvisionerによる処理方法は，Vagrantfileに記述されている．このために，Vagrantを用いれば，チームメンバーが同じソフトウェアの下で，仮想サーバを構築し，ソフトウェアをインストールすることができる．サーバの情報は，ENVファイルで以下の様に管理する．
...
| **```vagrant up```**            | サーバ起動                 |
| **```vagrant halt```**          | サーバ停止                 |
| **```vagrant ssh```**           | 仮想マシンへのリモート接続 |
| **```vagrant global-status```** | 起動中サーバの一覧         |



### :pushpin: Providerによる仮想マシン内のサーバの構築
...
Providerによって構築された仮想マシン内のサーバに，Web開発のためのソフトウェアをインストールするソフトウェア．具体的には，プログラミング言語やファイアウォールをインストールする．
...
クライアントは，Dockerを介して，コンテナを操作することになる．
...
1. DockerHubから，ベースイメージをインストールする．
...
### :pushpin: ベースイメージのインストール

- **Dockerイメージとベースイメージの対応例**
...
| コマンド                      | 処理                                           |
| ----------------------------- | ---------------------------------------------- |
| **```search {イメージ名}```** | レジストリに保管されているイメージを検索       |
| **```pull {イメージ名}```**   | イメージをレジストリからインストール           |
| **```push {イメージ名}```**   | イメージをレジストリにアップロード             |
| **```images```**              | クライアントにインストールされたイメージを確認 |




### :pushpin: Dockerイメージのビルド
...
イメージレイヤーからなるDockerイメージをビルドするための実行ファイル．Dockerイメージは，OSのイメージをベースイメージとしたイメージレイヤーになっている．以下のコマンドで，指定したDockerイメージのみをビルドできる．
...
以下の5つ順番で命令を行う．命令は，慣例的に大文字で記述する．
...
命令によって実行するコマンドの書き方には，Shell形式，Exec形式がある．以下では，Shell形式で記述する．
