...
プログラム言語のソースコードは、言語プロセッサによって機械語に変換された後、CPUによって読み込まれる。そして、ソースコードに書かれた様々な処理が実行される。コンパイラという言語プロセッサによって翻訳される言語をコンパイラ型言語、インタプリタという言語プロセッサによって翻訳される言語をインタプリタ型言語という。
...
### ◆ コンパイラ型とインタプリタ型の中間型であるJavaの翻訳

1. ソースコードを、クラスファイルに変換する。
2. JVM：Java Virtual Machine内で、クラスファイルをクラスデータに変換する。
3. インタプリタによって、クラスデータを機械語に翻訳する。
