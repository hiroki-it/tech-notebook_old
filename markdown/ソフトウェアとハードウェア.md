...
- [01-01. ソフトウェアの具体例](#01-01-ソフトウェアの具体例)
        - [◆ OSS：Open Source Softwareとは](#◆-ossopen-source-softwareとは)
        - [◆ OSSの具体例](#◆-ossの具体例)
        - [◆ Linux Distribution（Linuxの種類）](#◆-linux-distributionlinuxの種類)
- [01-02. ソフトウェアについて](#01-02-ソフトウェアについて)
        - [◆ ユーザの操作が、命令としてハードウェアに伝わるまで](#◆-ユーザの操作が命令としてハードウェアに伝わるまで)
        - [◆ 応用ソフトウェア](#◆-応用ソフトウェア)
        - [◆ Middleware](#◆-middleware)
        - [◆ 基本ソフトウェア（広義のOS）](#◆-基本ソフトウェア広義のos)
        - [◆ Firmware](#◆-firmware)
        - [◆ デバイスドライバ](#◆-デバイスドライバ)
- [01-03. 応用ソフトウェアの種類](#01-03-応用ソフトウェアの種類)
        - [◆ 応用ソフトウェア（アプリケーション）の一覧](#◆-応用ソフトウェアアプリケーションの一覧)
        - [◆ ネイティブアプリケーション](#◆-ネイティブアプリケーション)
        - [◆ Webアプリケーションとクラウドアプリケーション](#◆-webアプリケーションとクラウドアプリケーション)
        - [◆ ハイブリッドアプリケーション](#◆-ハイブリッドアプリケーション)
- [01-04. 基本ソフトウェアの汎用言語プログラム](#01-04-基本ソフトウェアの汎用言語プログラム)
        - [◆ ユーザの操作が、命令としてハードウェアに伝わるまで（再掲）](#◆-ユーザの操作が命令としてハードウェアに伝わるまで再掲)
        - [◆ 基本ソフトウェア（再掲）](#◆-基本ソフトウェア再掲)
        - [◆ コンパイラ型言語、インタプリタ型言語、JavaまたはJava仮想マシン型言語](#◆-コンパイラ型言語インタプリタ型言語javaまたはjava仮想マシン型言語)
        - [◆ コンパイラによるコンパイラ型言語の翻訳（じ、こ、い、さい、せい、リンク、実行）](#◆-コンパイラによるコンパイラ型言語の翻訳じこいさいせいリンク実行)
        - [◆ インタプリタによるインタプリタ型言語の翻訳（じ、こ、い、実行）](#◆-インタプリタによるインタプリタ型言語の翻訳じこい実行)
        - [◆ Java仮想マシンによるJavaまたはJava仮想マシン型言語の翻訳](#◆-java仮想マシンによるjavaまたはjava仮想マシン型言語の翻訳)
- [01-05. 基本ソフトウェアの制御プログラム（カーネル）](#01-05-基本ソフトウェアの制御プログラムカーネル)
        - [◆ ユーザの操作が、命令としてハードウェアに伝わるまで（再掲）](#◆-ユーザの操作が命令としてハードウェアに伝わるまで再掲-1)
        - [◆ 基本ソフトウェア（再掲）](#◆-基本ソフトウェア再掲-1)
        - [◆ ジョブ管理](#◆-ジョブ管理)
        - [◆ マスタスケジュラ、ジョブスケジュラ](#◆-マスタスケジュラジョブスケジュラ)
        - [◆ Initiatorによるジョブのジョブステップへの分解](#◆-initiatorによるジョブのジョブステップへの分解)
        - [◆ タスク管理](#◆-タスク管理)
        - [◆ 入出力管理](#◆-入出力管理)
- [01-06. デバイスドライバ](#01-06-デバイスドライバ)
- [01-07. Firmware](#01-07-firmware)
        - [◆ BIOS：Basic Input/Output System](#◆-biosbasic-inputoutput-system)
        - [◆ UEFI：United Extensible Firmware Interface](#◆-uefiunited-extensible-firmware-interface)
- [02-01. ハードウェアについて](#02-01-ハードウェアについて)
        - [◆ ユーザの操作が、命令としてハードウェアに伝わるまで（再掲）](#◆-ユーザの操作が命令としてハードウェアに伝わるまで再掲-2)
        - [◆ CPU（プロセッサ）](#◆-cpuプロセッサ)
        - [◆ RAM（メインメモリ＋キャッシュメモリ）](#◆-ramメインメモリ＋キャッシュメモリ)
        - [◆ ROM](#◆-rom)
        - [◆ 入力装置](#◆-入力装置)
        - [◆ 出力装置](#◆-出力装置)
- [02-02. CPU（プロセッサ）](#02-02-cpuプロセッサ)
        - [◆ IntelとAMDにおけるCPUの歴史（※2009年まで）](#◆-intelとamdにおけるcpuの歴史※2009年まで)
        - [◆ クロック周波数](#◆-クロック周波数)
        - [◆ MIPS：Million Instructions Per Second（×10^6 命令数／秒）](#◆-mipsmillion-instructions-per-second×10^6-命令数／秒)
- [02-03. 物理メモリ（RAM + ROM）](#02-03-物理メモリram--rom)
        - [◆ 物理メモリの種類](#◆-物理メモリの種類)
        - [◆ RAM：Read Access Memory](#◆-ramread-access-memory)
        - [◆ ROM：Read Only Memory](#◆-romread-only-memory)
        - [◆ Garbage collection](#◆-garbage-collection)
- [02-04. SRAM](#02-04-sram)
        - [◆ キャッシュメモリとは](#◆-キャッシュメモリとは)
        - [◆ キャッシュメモリの読み込み方法](#◆-キャッシュメモリの読み込み方法)
        - [◆ キャッシュメモリへの書き込み方式の種類](#◆-キャッシュメモリへの書き込み方式の種類)
        - [◆ 実効アクセス時間](#◆-実効アクセス時間)
...
- [02-06. ページング方式](#02-06-ページング方式)
        - [◆ ページ構造](#◆-ページ構造)
        - [◆ ページインとページアウト](#◆-ページインとページアウト)
        - [◆ 仮想メモリとのマッピングによる大容量アドレス空間の実現](#◆-仮想メモリとのマッピングによる大容量アドレス空間の実現)
        - [◆ MMU：Memory Management Unit（メモリ管理ユニット）](#◆-mmumemory-management-unitメモリ管理ユニット)
- [02-07. 物理メモリのアドレス空間管理](#02-07-物理メモリのアドレス空間管理)
        - [◆ 固定区画方式（同じ大きさの区画に分割する方式）](#◆-固定区画方式同じ大きさの区画に分割する方式)
        - [◆ 可変区画方式（様々な大きさの区画に分割する方式）](#◆-可変区画方式様々な大きさの区画に分割する方式)
        - [◆ オーバーレイ方式](#◆-オーバーレイ方式)
        - [◆ フラグメンテーション／メモリコンパクション](#◆-フラグメンテーション／メモリコンパクション)
        - [◆ スワッピング方式](#◆-スワッピング方式)
- [02-08. Page fault発生時の処理](#02-08-page-fault発生時の処理)
        - [◆ Page faultとは](#◆-page-faultとは)
        - [◆ Page Replacementアルゴリズム](#◆-page-replacementアルゴリズム)
- [02-09. アドレス空間管理におけるプログラムの種類](#02-09-アドレス空間管理におけるプログラムの種類)
        - [◆ Reusable（再使用可能プログラム）](#◆-reusable再使用可能プログラム)
        - [◆ Reentrant（再入可能プログラム）](#◆-reentrant再入可能プログラム)
        - [◆ Relocatable（再配置可能プログラム）](#◆-relocatable再配置可能プログラム)
- [02-10. ディスクメモリ](#02-10-ディスクメモリ)
        - [◆ ディスクメモリの機能](#◆-ディスクメモリの機能)
- [02-11. ハードディスク](#02-11-ハードディスク)
        - [◆ Defragmentation](#◆-defragmentation)
        - [◆ RAID：Redundant Arrays of Inexpensive Disks](#◆-raidredundant-arrays-of-inexpensive-disks)
- [02-12. GPUとVRAM](#02-12-gpuとvram)
- [03-01. 入出力装置](#03-01-入出力装置)
        - [◆ キーボードからポインティングデバイス](#◆-キーボードからポインティングデバイス)
        - [◆ 読み取り装置](#◆-読み取り装置)
        - [◆ ディスプレイ](#◆-ディスプレイ)
        - [◆ プリンタ](#◆-プリンタ)
- [03-02 入出力インターフェイス](#03-02-入出力インターフェイス)
        - [◆ Serial interface vs. Parallel interface](#◆-serial-interface-vs-parallel-interface)
        - [◆ Serial interface が用いられている例](#◆-serial-interface-が用いられている例)
        - [◆ Parallel interface が用いられている例](#◆-parallel-interface-が用いられている例)
        - [◆ 無線インターフェイス](#◆-無線インターフェイス)
- [04-01. 組み込み機器と組み込みシステム](#04-01-組み込み機器と組み込みシステム)
        - [◆ 組み込みシステムとは](#◆-組み込みシステムとは)
        - [◆ マイクロコンピュータとは](#◆-マイクロコンピュータとは)
        - [◆ センサによるアナログ情報の入力](#◆-センサによるアナログ情報の入力)
        - [◆ A/D変換器によるアナログ情報からデジタル情報への変換](#◆-ad変換器によるアナログ情報からデジタル情報への変換)
        - [◆ **D/A変換器**によるデジタル情報からアナログ情報への変換](#◆-da変換器によるデジタル情報からアナログ情報への変換)
        - [◆ Actuater](#◆-actuater)
        - [◆ 組み込みシステムの制御方式の種類](#◆-組み込みシステムの制御方式の種類)
...
ページアウトのアルゴリズムのこと。アルゴリズムの各方式で、物理メモリのページフレームからハードディスクにページアウトするページが異なる。
