...
| コマンド                             | 処理                                                         | 備考                                                         |
| :----------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **```search {イメージ名}```**        | ホスト側のイメージを検索                                     |                                                              |
| **```pull {イメージ名}```**          | イメージをホスト側にインストール                             |                                                              |
| **```image ls```**                   | ホスト側にインストールされたイメージを確認                   |                                                              |
| **```build --no-cache```**           | キャッシュ無しでDockerfileを基にイメージをビルド             |                                                              |
| **```up -d```**                      | ・Dockerfileを基にイメージのビルド<br>・全コンテナを構築<br>・コンテナを起動 | すでにコンテナがある場合，それを再起動する                   |
| **```run```**                        | ・Dockerfileを基にイメージをビルド<br>・指定／依存コンテナを構築<br>・コンテナを起動 | すでにコンテナがあっても，それを残して構築／起動．<br>（注）不要なコンテナが増える． |
| **```start```**                      | 既存コンテナを起動                                           |                                                              |
| **```stop```**                       | 起動中コンテナを停止                                         |                                                              |
...
Dockerデーモンを経由して，dockerホストの操作を行う．
...
インストールされたDockerイメージから，イメージのビルド，コンテナの構築，コンテナの起動，を行うための管理ツール．
