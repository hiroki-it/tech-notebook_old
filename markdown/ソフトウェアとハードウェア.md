...
ProviderとProvisionerを操作し，仮想マシン内のサーバ，ソフトウェアのインストール，を自動化するソフトウェア．チームメンバーが別々に仮想サーバを構築する場合，ProviderとProvisionerの処理によって作られる仮想サーバの環境に，違いが生じてしまう．Vagrantを使う場合，ProviderとProvisionerによる処理方法は，Vagrantfileに記述されている．このために，Vagrantを用いれば，チームメンバーが同じソフトウェアの下で，仮想マシン内にサーバを構築し，ソフトウェアをインストールすることができる．サーバの情報は，ENVファイルで以下の様に管理する．
...
|                                    | コマンド                                        |
| :--------------------------------: | ----------------------------------------------- |
| **イメージのキャッシュ無しビルド** | ```docker-compose build --no-cache```           |
| **イメージからコンテナ構築と起動** | ```docker-compose up -d```                      |
|          **コンテナ起動**          | ```docker-compose start```                      |
|          **コンテナ停止**          | ```docker-compose stop```                       |
|          **リモート接続**          | ```docker-compose exec -it {コンテナ名} bash``` |
|       **起動とリモート接続**       | ```docker-compose run```と既存コンテナ削除          |
|      **起動中コンテナの一覧**      | ```docker-compose ps```                         |
