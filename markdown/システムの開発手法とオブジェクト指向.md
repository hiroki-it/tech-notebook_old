...
![ウォーターフォール型](C:\Projects\tech-notebook\markdown\image\ウォーターフォール型.png)

- **外部設計の詳細**

  外部設計では、ユーザ向けのシステム設計が行われる。

  ![外部設計の詳細](C:\Projects\tech-notebook\markdown\image\外部設計の詳細.png)
...
![p456](C:\Projects\tech-notebook\markdown\image\p456.png)
...
![p458](C:\Projects\tech-notebook\markdown\image\p458.png)
...
![p462-1](C:\Projects\tech-notebook\markdown\image\p462-1.png)

![p462-2](C:\Projects\tech-notebook\markdown\image\p462-2.png)
...
![p457](C:\Projects\tech-notebook\markdown\image\p457.png)
...
![p463](C:\Projects\tech-notebook\markdown\image\p463.png)
...
![p459](C:\Projects\tech-notebook\markdown\image\p459.png)
...
![p491-3](C:\Projects\tech-notebook\markdown\image\p491-3.jpg)
...
![単体テスト](C:\Projects\tech-notebook\markdown\image\p490-1.jpg)

- **ブラックボックステスト**

  モジュール内の実装内容は気にせず、入力に対して、適切な出力が行われているかを検証。

  ![p492-1](C:\Projects\tech-notebook\markdown\image\p492-1.jpg)

- **ホワイトボックステスト**

  モジュール内の実装内容が適切かを確認しながら、入力に対して、適切な出力が行われているかを検証。

  ![p492-2](C:\Projects\tech-notebook\markdown\image\p492-2.jpg)
...
![p494-1](C:\Projects\tech-notebook\markdown\image\p494-1.png)
...
![p494-2](C:\Projects\tech-notebook\markdown\image\p494-2.png)
...
![p494-3](C:\Projects\tech-notebook\markdown\image\p494-3.png)
...
![p494-4](C:\Projects\tech-notebook\markdown\image\p494-4.png)
...
![結合テスト](C:\Projects\tech-notebook\markdown\image\p491-1.jpg)

- **Top-down テスト**

  上位のモジュールから下位のモジュールに向かって、結合テストを行う場合、下位には Stub と呼ばれるダミーモジュールを作成する。

  ![トップダウンテスト](C:\Projects\tech-notebook\markdown\image\トップダウンテスト.jpg)

- **Bottom-up テスト**

  下位のモジュールから上位のモジュールに向かって、結合テストを行う場合、上位には Driver と呼ばれるダミーモジュールを作成する。

  ![ボトムアップテスト](C:\Projects\tech-notebook\markdown\image\ボトムアップテスト.jpg)
...
![p491-2](C:\Projects\tech-notebook\markdown\image\p491-2.jpg)
...
![p496](C:\Projects\tech-notebook\markdown\image\p496.jpg)
...
![品質管理図](C:\Projects\tech-notebook\markdown\image\品質管理図.JPG)

不良摘出実績線（信頼度成長曲線）は、プログラムの品質の状態を表し、S字型でないものはプログラムの品質が良くないことを表す。

![信頼度成長曲線の悪い例](C:\Projects\tech-notebook\markdown\image\信頼度成長曲線の悪い例.JPG)
...
- **プロジェクトに必要な日数**
  ...
  ![p509](C:\Projects\tech-notebook\markdown\image\p509.jpg)

- **最早結合点時刻**

  全体的な工程の中で、任意の結合点に取り掛かるために必要な最少日数のこと。Critical Path に影響されるので、注意。

  ![p510-1](C:\Projects\tech-notebook\markdown\image\p510-1.jpg)

- **最遅結合点時刻**

  全体的な工程の中で、任意の結合点に取り掛かるために必要な最多日数のこと。

  ![p510-2](C:\Projects\tech-notebook\markdown\image\p510-2.jpg)
...
![構造化プログラミングからオブジェクト指向へ](C:\Projects\tech-notebook\markdown\image\構造化プログラミングからオブジェクト指向へ.png)
...
オブジェクトモデリングの方法として、『実体』の『状態』と『動作』を考える。しかし、これは厳密ではない。なぜなら、ビジネス領域を実装する時には、ほとんどの場合で、動作を持たない実体を表現することになるからである。より厳密に理解するために、『実体』『状態』『状態の編集と表示』と考えるべき。
...
![複数視点のモデル化とUML](C:\Projects\tech-notebook\markdown\image\複数視点のモデル化とUML.jpg)
...
![UML-0](C:\Projects\tech-notebook\markdown\image\UML-0.png)

### ◆ UMLのダイアグラム一覧

![UML-1](C:\Projects\tech-notebook\markdown\image\UML-1.png)

![UML-2](C:\Projects\tech-notebook\markdown\image\UML-2.png)
...
![クラス図の線の種類](C:\Projects\tech-notebook\markdown\image\クラス図の線の種類.png)

- **Class**

１つのクラスを、クラス区画、属性区画、操作区画の３要素で表記する方法。

![UML](C:\Projects\tech-notebook\markdown\image\クラス図.png)

- **Association（関連）**

２つのクラスを関連させる場合、クラスを線で繋ぐことで関連性を表記する方法。クラス図の実装の章を参照せよ。

![クラス図の関連表現](C:\Projects\tech-notebook\markdown\image\クラス図の関連表現.png)
...
- **Aggregation（集約）**
  ...
  **【具体例】**
  ...
  社員は１つの会社に所属する場合
    ...
    「社員クラス」から見た「会社クラス」の数は1つである。逆に、「会社クラス」からみた「社員クラス」は0以上であるという表記。

![集約](C:\Projects\tech-notebook\markdown\image\集約.png)
...
![汎化と特化](C:\Projects\tech-notebook\markdown\image\汎化と特化.png)
...
「社員クラス」から見た「会社クラス」の数は1つである。逆に、「会社クラス」からみた「社員クラス」の数は0以上であるという表記。

![多重度](C:\Projects\tech-notebook\markdown\image\多重度.png)

![多重度一覧](C:\Projects\tech-notebook\markdown\image\多重度の表記方法.png)
...
![Association, Aggregation, Compositionの関係の強さの違い](C:\Projects\tech-notebook\markdown\image\Association, Aggregation, Compositionの関係の強さの違い.png)
...
![抽象クラス](C:\Projects\tech-notebook\markdown\image\抽象クラス.png)
...
![インターフェースとは](C:\Projects\tech-notebook\markdown\image\インターフェースとは.png)

実装クラスに処理内容を記述しなければならない。すなわち、抽象クラスにメソッドの型のみ定義した場合と同じである。多重継承できる。

![子インターフェースの多重継承_2](C:\Projects\tech-notebook\markdown\image\子インターフェースの多重継承_2.png)
...
| -------------------- | :--------: | :------------------------: | :------------------------: |
| 役割                 |   クラス   | 鋳型：クラス全体の型を提供 | 機能箱：メソッドのみを提供 |
| 子クラスでの継承先数 |  単一継承  |          単一継承          |     単一継承｜多重継承     |
| メンバ変数のコール   |  自クラス  |     継承先子クラスのみ     |     実装先子クラスのみ     |
| 定数の定義           |     〇     |             〇             |             〇             |
| 抽象メソッドの定義   |     ✕      |             〇             |             〇             |
| 具象メソッドの定義   |     〇     |             〇             |             ✕              |
| construct() の定義   |     〇     |             〇             |             ✕              |
...
![インターフェースと抽象クラスの使い分け](C:\Projects\tech-notebook\markdown\image\インターフェースと抽象クラスの使い分け.png)
...
![トレイト](C:\Projects\tech-notebook\markdown\image\トレイト.png)
...
![ER図（IE記法）](C:\Projects\tech-notebook\markdown\image\ER図（IE記法）.png)

- **Entity と Attribute**

  ![エンティティとアトリビュート](C:\Projects\tech-notebook\markdown\image\エンティティとアトリビュート.png)

- **Relation と Cardinality（多重度）**

  エンティティ間の関係を表す。

  ![リレーションとカーディナリティ](C:\Projects\tech-notebook\markdown\image\リレーションとカーディナリティ.png)

- **1：1**

  ![１：１](C:\Projects\tech-notebook\markdown\image\１：１.png)

- **1：多（Relation が曖昧な状態）**

  設計が進むにつれ、「1：0 以上の関係」「1：1 以上の関係」のように具体化しく。

  ![１：多（Relationが曖昧な状態）](C:\Projects\tech-notebook\markdown\image\１：多（Relationが曖昧な状態）.png)

- **1：1 以上**

  ![１：１以上](C:\Projects\tech-notebook\markdown\image\１：１以上.png)
...
![アクティビティ図](C:\Projects\tech-notebook\markdown\image\アクティビティ図.png)

- **アルゴリズムとフローチャート**

  ![p549-1](C:\Projects\tech-notebook\markdown\image\p549-1.gif)

  ![p549-2](C:\Projects\tech-notebook\markdown\image\p549-2.gif)

  ![p549-3](C:\Projects\tech-notebook\markdown\image\p549-3.gif)
...
![データフロー図](C:\Projects\tech-notebook\markdown\image\データフロー図.jpg)
...
![シーケンス図](C:\Projects\tech-notebook\markdown\image\シーケンス図.png)
...
![状態遷移図](C:\Projects\tech-notebook\markdown\image\ストップウォッチ状態遷移図.jpg)
...
![状態遷移表](C:\Projects\tech-notebook\markdown\image\ストップウォッチ状態遷移表.jpg)
...
![状態遷移表](C:\Projects\tech-notebook\markdown\image\状態遷移表.png)
...
![ネイティブアプリ](C:\Projects\tech-notebook\markdown\image\ネイティブアプリ.png)
...
![Webアプリ](C:\Projects\tech-notebook\markdown\image\Webアプリ.png)
...
![Webviewよるアプリパッケージ](C:\Projects\tech-notebook\markdown\image\Webviewよるアプリパッケージ.png)

![ハイブリッドアプリ](C:\Projects\tech-notebook\markdown\image\ハイブリッドアプリ.png)
