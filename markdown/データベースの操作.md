...
![データベース管理システムの仕組み](image\データベース管理システムの仕組み.png)
...
![カラムの増減は✖、レコードの増減は〇](image\カラムの増減は✖、レコードの増減は〇-1.png)

**【具体例】**

賞与を年1回から、2回・3回と変える場合、主キーを繰り返し、新しく賞与区分と金額区分を作る。

![カラムの増減は✖、レコードの増減は〇-2](image\カラムの増減は✖、レコードの増減は〇-2.png)
...
![第三正規形](image\第三正規形-1.png)

![第三正規形-2](image\第三正規形-2.png)

5. **まとめ**

   主キーの関係従属性の排除によって、受注表、商品表、数量表に分割できた。また、主キー以外の関係従属性の排除によって、顧客IDを新しい主キーとした顧客表に分割できた。

![正規化後にどんな表ができるのか](image\正規化後にどんな表ができるのか.png)
...
![非正規形-2](image\非正規形-2.png)

2. **第一正規化（繰り返し要素の排除）**

   ![第一正規形-2](image\第一正規形-2.png)



# 02-01. 関係データベースに必要な機能
...
![DBMSによるメモリとディスクの使い分け](image\DBMSによるメモリとディスクの使い分け.jpg)

そして、チェックポイントで、ログファイルの更新後ログをディスク上のデータファイルに反映させる。この時、チェックポイントは、自動実行または手動実行で作成する。

![トランザクション](image\トランザクション.jpg)
...
![障害回復機能](image\システム障害の障害回復機能.jpg)
...
![媒体障害の障害回復機能](image\媒体障害の障害回復機能.jpg)
...
![排他制御-1](image\排他制御-1.png)

- **排他制御を行った結果**

  ![排他制御-2](image\排他制御-2.png)

  

### ◆ 排他制御におけるロックの種類

![排他制御-3](image\排他制御-3.gif)
...
![マッチング処理_1](image\マッチング処理_1.PNG)



### ◆ 突き合わせ処理のアルゴリズム

![マッチング処理_4](image\マッチング処理_4.png)
