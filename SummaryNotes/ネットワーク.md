...
# 02-05. 【TCP/IP】アプリケーション層の『SMTP』『POP3』『IMAP』
...
ファイアウォールとして用いられる。パケットのヘッダ情報に記載された送信元IPアドレスやポート番号などによって、パケットを許可するべきかどうかを決定する。
...
### ◇ プライベートネットワークにおけるファイアウォールの種類

- **プライベートネットワークにおけるファイアウォール（再掲）**

![内部、DMZ、外部](C:\Projects\summary_notes\SummaryNotes\Image\内部、DMZ、外部.png)

- **パケットフィルタリング型ファイアウォール（再掲）**

プロキシサーバのパケットフィルタリング機能をファイアウォールとして用いる。パケットのヘッダ情報に記載された送信元IPアドレスやポート番号などによって、パケットを許可するべきかどうかを決定する。速度を重視する場合はこちら。
...
従来はパッケージとして提供されていたソフトウェアを、インターネット上で利用出来るようにしたサービスのこと。

- **Google Apps（Google Map、Google Cloud、Google Calender など）**



# 03-02. GitHubからAWS仮想サーバへのデプロイ
...
クラウドプライベートネットワークとして働く。AWSでは、プライベートIPアドレスが割り当てられた、VPCと呼ばれるプライベートネットワークを仮想的に構築することができる。
