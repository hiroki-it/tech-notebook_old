...
![WAN、LANの具体例](D:\Documents\Drive 1st\プログラミング自習\まとめノート\随時、リポジトリの画像フォルダへコピーせよ\WAN、LANの具体例.png)



### ◇ WANの種類と歴史

![WANの歴史](D:\Documents\Drive 1st\プログラミング自習\まとめノート\随時、リポジトリの画像フォルダへコピーせよ\WANの種類と歴史.png)
...
![プロキシサーバー](C:\Projects\Summary_Notes\まとめノート\画像\プロキシサーバー.png)
...
![ネットワークにおけるTCP_IPを用いたデータ通信](D:\Documents\Drive 1st\プログラミング自習\まとめノート\随時、リポジトリの画像フォルダへコピーせよ\ネットワークにおけるTCP_IPを用いたデータ通信.png)
...
1. ヘッダー：リクエストの詳細情報
1. メソッド：GET
1. リクエストの対象:ジャンプするURLを指定
1. HTTPのバージョン
1. User-Agent：ブラウザのバージョン情報等
1. Accept：ブラウザが想定する(利用可能な)MIMEのタイプ
1. Referer：遷移元のページ
1. Accept-Encoding：ブラウザがデコードできるエンコーディング形式
1. Accept-Language：ブラウザが想定する(利用可能な)言語
...
1. リクエスト行：常に１行
1. ヘッダー：リクエストの詳細情報がある
1. 空白行：ヘッダーとボディを分ける
1. ボディ：画面での入力内容が入っている
1. メソッド：POST
1. リクエストの対象：ジャンプするURLを指定
1. HTTPのバージョン
...
1. ステータスコード：200はサーバーのWebシステム処理が成功したことを表す

1. Server：サーバーの名前とバージョン等です。

1. Content-Type：出力するMIMEタイプ
...
![デジタル証明書によるなりすまし防止3,4](C:\Projects\Summary_Notes\まとめノート\画像\デジタル証明書によるなりすまし防止-3,4.png)<u></u>
